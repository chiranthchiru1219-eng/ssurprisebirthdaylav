...
function initVideoParticles() {
  ...
  let effectContainer = document.getElementById('cinematic-dust-container');
  if (!effectContainer) {
      effectContainer = document.createElement('div');
      effectContainer.id = 'cinematic-dust-container';
      ...
      effectContainer.style.mixBlendMode = 'multiply';
  ...
  // Create Watercolor Ink/Petal Particles
  const particleCount = 15; 
  // Deep Crimson, Muted Green, Soft Pink
  const colors = ['rgba(163, 22, 33, 0.3)', 'rgba(46, 74, 62, 0.2)', 'rgba(233, 30, 99, 0.15)'];

  for (let i = 0; i < particleCount; i++) {
    const p = document.createElement('div');
    p.className = 'watercolor-particle';
    ...
    // Reduced blur for visibility (Soft watercolor edge)
    p.style.filter = `blur(${Math.random() * 4}px)`;
    
    // Animation Properties
    const duration = 30 + Math.random() * 30;
    p.style.animation = `inkFloat ${duration}s ease-in-out infinite alternate`;
...
function stopVideoParticles() {
    const tunnelContainer = document.getElementById('cinematic-dust-container');
